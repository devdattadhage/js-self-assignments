function getSingleDigitInWord(number) {
...
function get11To19InWord(number) {
  const elevenths = ['eleven', 'twelve', 'thirteen', 'fourteen', 'fifteen',
    'sixteen', 'seventeen', 'eighteen', 'nineteen'];

  return elevenths[number - 1];
}

function getTensDigitInWord(number) {
...
function _getTwoDigitInWord(number, tensNum, unitNum) {
  if (number % 10 === 0) {
    return getTensDigitInWord(tensNum);
  }
  if (number < 20) {
    const isTrue = tensNum === 1;
    return isTrue ? get11To19InWord(unitNum) : getSingleDigitInWord(unitNum);
  }

  return getTensDigitInWord(tensNum) + ' ' + getSingleDigitInWord(unitNum);
}

function getTwoDigitInWord(num) {
  const number = +(num.slice(num.length - 2, num.length));
  const tensNum = +num[num.length - 2];
  const unitNum = +num[num.length - 1];

  if (number === 0) {
    return "";
  }

  return _getTwoDigitInWord(number, tensNum, unitNum);
}

function getThreeDigitInWord(num) {
  const hundredNum = +num[num.length - 3];

  if (hundredNum === 0) {
    return getTwoDigitInWord(num);
  }

  const hundred = getSingleDigitInWord(hundredNum) + ' hundred ';

  return (hundred + getTwoDigitInWord(num)).trim();
}

function getThousandsInWord(num) {
  let thousandNum = +(num.slice(num.length - num.length, num.length - 3));

  if (thousandNum === 0) {
    return getThreeDigitInWord(num);
  }

  const isTrue = thousandNum < 100;
  thousandNum += "";
  const thousand = isTrue ?
    getTwoDigitInWord(thousandNum) : getThreeDigitInWord(thousandNum);

  return (thousand + " thousand " + getThreeDigitInWord(num)).trim();
}

function getMillionsInWord(num) {
  let millionNum = +(num.slice(num.length - num.length, num.length - 6));
  const thousandNum = (num.slice(num.length - 6, num.length));

  if (millionNum === 0) {
    return getThousandsInWord(thousandNum);
  }

  const isTrue = millionNum < 100;
  millionNum += "";
  const million = isTrue ?
    getTwoDigitInWord(millionNum) : getThreeDigitInWord(millionNum);

  return (million + " million " + getThousandsInWord(thousandNum)).trim();
}

function getBillionsInWord(num) {
  let billionNum = +(num.slice(num.length - num.length, num.length - 9));
  const millionNum = (num.slice(num.length - 9, num.length));

  if (billionNum === 0) {
    return getMillionsInWord(millionNum);
  }

  const isTrue = billionNum < 100;
  billionNum += "";
  const billion = isTrue ?
    getTwoDigitInWord(billionNum) : getThreeDigitInWord(billionNum);

  return (billion + " billion " + getMillionsInWord(millionNum)).trim();
}

function conversionOf0To999(number) {
  if (number.length === 1) {
    return getSingleDigitInWord(+number);
  }
  if (number.length === 2) {
    return getTwoDigitInWord(number);
  }

  return getThreeDigitInWord(number);
}

function conversionOf999Above(number) {
  if (number.length > 3 && number.length < 7) {
    return getThousandsInWord(number);
  }
  if (number.length > 6 && number.length < 10) {
    return getMillionsInWord(number);
  }

  return getBillionsInWord(number);
}
...
function numberToWords(num) {
  ...
  numberInWords = number.length < 4 ?
    conversionOf0To999(number) : conversionOf999Above(number);
